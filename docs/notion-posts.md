...
段落、标题（H1–H3）、列表、引用、代码、图片（含 caption → `<figure class="figure-image">`）、链接、简单表格、分隔线（`divider` → `<hr>`）、分栏（`column_list` / `column`：桌面并排、手机单列；纯图和混排正文都保持分栏，不再摊平）、callout（当引用）、同步块（摊平子块）。
