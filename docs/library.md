...
首屏 HTML 只放当前排序下的 **30** 条。其余条目在构建时写进页面里的 JSON，「加载更多」再按 30 条挂进 DOM。切换类型时，用这份 JSON 重新匹配，不依赖已经出现在页面上的卡片。

点击卡片打开浅色详情弹层（封面、标题、评分、短评、链接、创建时间），不离开本页。类型是「游戏」且填了「游戏时长（小时）」时，弹层多一行「游戏时长」，例如「12 小时」或「1.5 小时」。Esc、点遮罩、或「关闭」都会关掉，并把焦点还回那张卡片。没有评分、链接、创建时间，或游戏没有时长时，对应一行不显示；非游戏即使填了时长也不显示。没有短评时写一句「还没有短评。」评分画成 5 颗星，颜色用站点 `--accent`。
...
| --- | --- | --- |
| 标题 | Title | 海报下方标题 |
| 类型 | Select：书 / 电影 / 剧集 / 综艺 / 音乐 / 游戏 | 筛选，以及 `类型 · 年份`。旧选项名「电视剧」在构建时归一成「剧集」，筛选「剧集」也会命中旧值 |
...
- `<img>` 带 2:3 的 `width` / `height`（360×540），`alt` 留空（标题在卡片文字里）。网格默认 `loading="lazy"`；首屏前 4 张 `eager`，第一张 `fetchpriority="high"`。弹层不懒加载。封面加载失败落到文字卡。
